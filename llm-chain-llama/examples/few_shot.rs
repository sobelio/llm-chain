...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res =
        Step::for_prompt_template(prompt!(user: user_prompt))
    ...
    let conversation = Conversation::new()
        ...
        .with_user_template(
            user_prompt,
            &parameters!().with("full_name", "Ding Liren"),
        )?
    ...
    let res = chain.send_message(step, &parameters!().with("full_name", "Elon Musk"), &exec_2).await?;
    println!(
        "{} (few-shot CoT answer)",
