...
async fn main() {
    ...
    let instr = new_instruct_template("Write a hypothetical weather report for {season} in {location}.");
    let chain = Step::new(
        instr
    )
    .to_chain();

    let res = chain.run(Parameters::new().with("season", "summer").with("location", "the moon"), exec).await.unwrap();
