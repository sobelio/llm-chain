...
///
/// let chat_gpt_step = Step::new(model, prompt, None);
/// ```
#[cfg_attr(feature = "serialization", derive(Serialize, Deserialize))]
pub struct Step {
...
impl Step {
    #[deprecated(since = "0.6.0", note = "Use `for_prompt_and_model` instead")]
    pub fn new<P: Into<ChatPromptTemplate>>(model: Model, prompt: P, stream: Option<bool>) -> Step {
    ...
    pub fn for_prompt_and_model<P: prompt::Prompt>(
        model: Model,
        prompt: P,
        stream: Option<bool>,
    ) -> Step {
        #[allow(deprecated)]
        Self::new(model, prompt.as_chat_prompt(), stream)
    }

    pub fn for_prompt<P: prompt::Prompt>(prompt: P) -> Step {
        Self::for_prompt_and_model(Model::ChatGPT3_5Turbo, prompt, None)
    }

    pub fn for_prompt_with_streaming<P: prompt::Prompt>(prompt: P) -> Step {
        Self::for_prompt_and_model(Model::ChatGPT3_5Turbo, prompt, Some(true))
    }
}
